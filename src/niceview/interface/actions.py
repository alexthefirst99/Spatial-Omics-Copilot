    ...
    if rois is not None and len(rois) > 0:
        ...
        for roi in rois:
            ...
            if roi.get("type") == "rect" or roi.get("type") == "polygon":
                # Rectangles may arrive as two corners or a full polygon.
                poly = [p for p in points if p]
                if poly and poly[0] != poly[-1]:
                    poly.append(poly[0])
                coords.append(poly)

        # Keep the original ROI alongside normalized pixel coordinates.
...
def clear_cache_forcall(*args):
    ...
    chat_dir = get_path("paths.chat_dir", os.path.join(data_dir, "chat_sessions"), env="COPILOT_CHAT_DIR")
    tmp_base = get_path("paths.tmp_base", os.path.join(project_root, "tmp_data"), env="COPILOT_TMP_BASE")

    # Delete all data derived from this browser session.
    if session_id:
