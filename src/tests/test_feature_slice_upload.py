...
def _write_coo_group(parent, name, rows, cols, values):
    ...
    group.create_dataset("row", data=np.asarray(rows, dtype=np.int64))
    group.create_dataset("col", data=np.asarray(cols, dtype=np.int64))
    group.create_dataset("data", data=np.asarray(values, dtype=np.int64))
    return group
